...
**Credits & blackjack**: Credits are persistent; new users start with 0. Earn via: Sub +100, Gift sub +100 (to gifter), Kicks +1 per kick, or `!addcredits user N` (broadcaster/mod). Spend on blackjack only. `!credits` or `!credits username` — check balance. `!lb` or `!leaderboard` — no leaderboards (reply: use !credits). **Blackjack**: `!deal 5` or `!bj 5` — start a hand. `!hit` `!stand` `!double` `!split`. Min bet 1. Blackjack pays 1.5×. No other games; no channel reward credits; no poll rewards in Phase 1.
